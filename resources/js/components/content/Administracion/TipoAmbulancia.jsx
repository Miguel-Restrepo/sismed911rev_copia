...
const notificarErrorCaso = () =>
    ...
    const Post = () => {
        axios.post('/api/tipo_ambulancia', form)
            .then(response => {
                notificarExitoCaso(response.data.id_tipotransport);
                Get()
                return response.data;
            })
            .catch(error => {
    ...
    const Edit = () => {
        axios.put(`/api/tipo_ambulancia/${line.id_tipotransport}`, form)
            .then(response => {
    ...
    const Elimina = () => {
        axios.delete(`/api/tipo_ambulancia/${line.id_tipotransport}/delete`)
            .then(response => {
                Get()
                setLine('')
                return response.data;
    ...
    const columns = [{
        dataField: 'id_tipotransport',
        text:  `${t("administracion.tipoamb.datos.id")}`,
        sort: true
    }, {
        dataField: 'tipo_amb_es',
        text: `${t("administracion.tipoamb.datos.tipo")}`,
        sort: true
    }, {
        text:'',
        dataField: '789',
            ...
            return (
                <div>
                    <Icofont icon="search-1" className="mx-2" onClick={() => {
                        handleView()
                        setForm({
                            tipo_amb_es: row.tipo_amb_es,
                            codigo: row.codigo
                        })
                        setLine({
                            id_tipotransport: row.id_tipotransport,
                            tipo_amb_es: row.tipo_amb_es,
                            codigo: row.codigo
                        })
                        handleShow();
    ...
    return (
        <div>
            <div>
                <h2>{t("administracion.tipoamb.titulo")}</h2>
            </div>
            <div>
                ...
                <Modal show={show} onHide={() => {
                    ...
                    <Modal.Header closeButton>
                        <Modal.Title> <h1>{t("administracion.tipoamb.titulo")} - {view ?  t("etiquetas.ver"): t("etiquetas.eliminar")}</h1></Modal.Title>
                    </Modal.Header>
                    ...
                    <Modal.Body>
                        ...
                        <Form>
                            <Form.Group as={Row} className="mb-3" controlId="formid">
                                ...
                                {t("administracion.tipoamb.datos.id")}
                                </Form.Label>
                                <Col sm="10">
                                    <Form.Control plaintext readOnly defaultValue={line.id_tipotransport} />
                                </Col>
                            </Form.Group>
                            <Form.Group as={Row} className="mb-3" controlId="formname">
                                <Form.Label column sm="2">
                                {t("administracion.tipoamb.datos.tipo")}
                                </Form.Label>
                                <Col sm="10">
                                    <Form.Control plaintext readOnly defaultValue={line.tipo_amb_es} />
                                </Col>
                            </Form.Group>
                        </Form>
                    </Modal.Body>
                ...
                <Modal show={showe} onHide={() => {
                    ...
                    <Modal.Header closeButton>
                        <Modal.Title>{t("administracion.tipoamb.titulo")}s - {editar ? t("etiquetas.editar") :  t("etiquetas.agregar")}</Modal.Title>
                    </Modal.Header>
                    ...
                    <Modal.Body>
                        ...
                        <Form className='m-xxl-4'>
                            <Form.Group as={Row} className="mb-3" controlId="formcodigo">
                                ...
                                <Col sm={4} >
                                    <Form.Control type='text' placeholder={`${t("administracion.tipoamb.datos.tipo")}`} value={form.tipo_amb_es} onChange={handleChange} name="tipo_amb_es" />
                                </Col>
                            </Form.Group>
