...
const ModalidadAmbulancia = () => {
    ...
    const Get = () => {
        axios.get(`/api/modalidad_ambulancia`)
            .then(response => {
...
const notificarErrorCaso = () =>
    ...
    const Post = () => {
        axios.post('/api/modalidad_ambulancia', form)
            .then(response => {
                notificarExitoCaso(response.data.id_modalidad);
                Get()
                return response.data;
            })
            .catch(error => {
    ...
    const Edit = () => {
        axios.put(`/api/modalidad_ambulancia/${line.id_modalidad}`, form)
            .then(response => {
    ...
    const Elimina = () => {
        axios.delete(`/api/modalidad_ambulancia/${line.id_modalidad}/delete`)
            .then(response => {
                Get()
                setLine('')
                return response.data;
    ...
    const columns = [{
        ...
        text: `${t("administracion.modalidad.datos.id")}`,
        sort: true
    }, {
        dataField: 'modalidadambu_es',
        text: `${t("administracion.modalidad.datos.nombre")}`,
        sort: true
    }, {
        text:'',
        dataField: '785',
            ...
            return (
                <div>
                    <Icofont icon="search-1" className="mx-2" onClick={() => {
                        handleView()
                        setForm({
                            modalidadambu_es: row.modalidadambu_es
                        })
                        setLine({
    ...
    return (
        <div>
            <div>
                <h2>{t("administracion.modalidad.titulo")}</h2>
            </div>
            <div>
                ...
                <Modal show={show} onHide={() => {
                    ...
                    <Modal.Header closeButton>
                        <Modal.Title> <h1>{t("administracion.modalidad.titulo")} - {view ?  t("etiquetas.ver") : t("etiquetas.eliminar")}</h1></Modal.Title>
                    </Modal.Header>
                    ...
                    <Modal.Body>
                        ...
                        <Form>
                            <Form.Group as={Row} className="mb-3" controlId="formid">
                                ...
                                {t("administracion.modalidad.datos.id")}
                                </Form.Label>
                                <Col sm="10">
                                    <Form.Control plaintext readOnly defaultValue={line.id_modalidad} />
                                </Col>
                            </Form.Group>
                            <Form.Group as={Row} className="mb-3" controlId="formname">
                                <Form.Label column sm="2">
                                {t("administracion.modalidad.datos.nombre")}
                                </Form.Label>
                                <Col sm="10">
                                    <Form.Control plaintext readOnly defaultValue={line.modalidadambu_es} />
                                </Col>
                            </Form.Group>
                        </Form>
                    </Modal.Body>
                ...
                <Modal show={showe} onHide={() => {
                    ...
                    <Modal.Header closeButton>
                        <Modal.Title>{t("administracion.modalidad.titulo")} - {editar ? t("etiquetas.editar") :  t("etiquetas.agregar")}</Modal.Title>
                    </Modal.Header>
                    ...
                    <Modal.Body>
                        ...
                        <Form className='m-xxl-4'>
                            <Form.Group as={Row} className="mb-3" controlId="formcodigo">
                                ...
                                <Col sm={4} >
                                    <Form.Control type='text' placeholder={`${t("administracion.modalidad.datos.nombre")}`} value={form.modalidadambu_es} onChange={handleChange} name="modalidadambu_es" />
                                </Col>
                            </Form.Group>
...
export default ModalidadAmbulancia;
