...
const notificarErrorCaso = () =>
    ...
    const Post = () => {
        axios.post('/api/code_planta', form)
            .then(response => {
                notificarExitoCaso(response.data.idacode);
                Get()
                return response.data;
            })
            .catch(error => {
    ...
    const Edit = () => {
        axios.put(`/api/code_planta/${line.idacode}`, form)
            .then(response => {
    ...
    const Elimina = () => {
        axios.delete(`/api/code_planta/${line.idacode}/delete`)
            .then(response => {
                Get()
                setLine('')
                return response.data;
    ...
    const columns = [{
        dataField: 'idacode',
        text: `${t("administracion.acode.datos.id")}`,
        sort: true
    }, {
        dataField: 'nombreacode',
        text: `${t("administracion.acode.datos.nombre")}`,
        sort: true
    }, {
        text:'',
        dataField: '78',
            ...
            return (
                <div>
                    <Icofont icon="search-1" className="mx-2" onClick={() => {
                        handleView()
                        setForm({ nombreacode: row.nombreacode })
                        setLine({
                            idacode: row.idacode,
                            nombreacode: row.nombreacode
                        })
                        handleShow();
                    }} />
                </div>
    ...
    return (
        <div>
            <div>
                <h2>{t("administracion.acode.titulo")}</h2>
            </div>
            <div>
                ...
                <Modal show={show} onHide={() => {
                    ...
                    <Modal.Header closeButton>
                        <Modal.Title> <h1>{t("administracion.acode.titulo")} - {view ? t("etiquetas.ver") : t("etiquetas.eliminar")}</h1></Modal.Title>
                    </Modal.Header>
                    ...
                    <Modal.Body>
                        ...
                        <Form>
                            <Form.Group as={Row} className="mb-3" controlId="formid">
                                ...
                                {t("administracion.acode.datos.id")}
                                </Form.Label>
                                <Col sm="10">
                                    <Form.Control plaintext readOnly defaultValue={line.idacode} />
                                </Col>
                            </Form.Group>
                            <Form.Group as={Row} className="mb-3" controlId="formname">
                                <Form.Label column sm="2">
                                {t("administracion.acode.datos.nombre")}
                                </Form.Label>
                                <Col sm="10">
                                    <Form.Control plaintext readOnly defaultValue={line.nombreacode} />
                                </Col>
                            </Form.Group>
                        </Form>
                    </Modal.Body>
                ...
                <Modal show={showe} onHide={() => {
                    ...
                    <Modal.Header closeButton>
                        <Modal.Title>{t("administracion.acode.titulo")} - {editar ? t("etiquetas.editar") : t("etiquetas.agregar")}</Modal.Title>
                    </Modal.Header>
                    ...
                    <Modal.Body>
                        ...
                        <Form className='m-xxl-4'>
                            <Form.Group as={Row} className="mb-3" controlId="formcodigo">
                                ...
                                <Col sm={4} >
                                    <Form.Control type='text' placeholder={`${t("administracion.acode.datos.nombre")}`} value={form.nombreacode} onChange={handleChange} name="nombreacode" />
                                </Col>
                            </Form.Group>
